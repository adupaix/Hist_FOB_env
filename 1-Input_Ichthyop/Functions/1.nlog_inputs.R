...
input.nlog <- function(DATA_PATH, OUTPUT_PATH, RESOURCE_PATH,
  ...
  if (all(files_to_check %in% list.files(dir_path)) & reset == F){
    
    cat("\n### Reading data from existing files\n")
    
    if (return_format == "mat"){
      output <- read.table(file = file.path(dir_path, "input_icht.txt"),
                           row.names = F, col.names = F, sep=" ")
    } else if (return_format == "sf"){
      output <- read_sf(file.path(dir_path, paste0(dir_name, ".shp")))
    }
